...
class WebAIBase(BaseAI):
    ...
    async def send_prompt(
        ...
        **kwargs
        ...
        success, snippet, markdown, meta = await self._execute_web_interaction(
            page, message, wait_for_response, timeout_s
        )

        # Update session stats if we have a successful interaction
        if success and (markdown or snippet):
            try:
                ...
                logger.debug(f"{self.ai_target}: session accounting failed: {e}")
