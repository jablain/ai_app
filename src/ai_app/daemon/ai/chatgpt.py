...
class ChatGPTAI(WebAIBase):
    ...
    INPUT_BOX = "textarea[name='prompt-textarea'], textarea[id='prompt-textarea']"
    ...
    MODEL_SELECTOR = "button[id*='headlessui-menu-button'], button[aria-controls*='model'], div[class*='model-switcher']"
    ...
    async def start_new_session(self) -> bool:
        try:
            ...
            # Try ChatGPT's new chat button first, then fallback to base URL
            try:
                new_btn = page.locator("a[href='/'] >> text=New chat, button:has-text('New chat')")
    ...
    async def list_messages(self) -> list[dict]:
        ...
        try:
            page = await self._get_or_open_page(self._get_page_url_hint())
            # Adjust selector to your ChatGPT response container constant
    ...
    async def _guess_model_name(self, page: Page) -> str | None:
        """
        ChatGPT-specific model name detection.
        
        Looks for model selector button with model name.
        """
        ...
        try:
            ...
            if await model_loc.count() > 0:
                ...
                # Extract model name (e.g., "GPT-4", "GPT-4 Turbo")
    ...
    async def _get_response_count(self, page: Page) -> int:
        ...
        Uses data-message-author-role to ensure we only count assistant messages.
        """
        try:
    ...
    async def _send_message(self, page: Page, message: str) -> bool:
        ...
        try:
            ...
            fb = page.locator("textarea, div[contenteditable='true'], div[role='textbox']")
            if await fb.count() > 0:
