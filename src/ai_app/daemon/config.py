...
def load_config() -> AppConfig:
    ...
    if not config_file.exists():
        _logger.info(
            ...
            "Create the file to customize settings.",
            config_file
        )
        return config
        ...
        if "cdp" in loaded:
            ...
            for key, value in cdp_data.items():
                if key == "health" and isinstance(value, dict):
                    ...
                    config.cdp.start_timeout_s = _validate_positive_float(timeout, "cdp.start_timeout_s")
                elif key == "probe_timeout_s":
                    timeout = _as_float(value, "cdp.probe_timeout_s")
                    config.cdp.probe_timeout_s = _validate_positive_float(timeout, "cdp.probe_timeout_s")
                elif key == "probe_interval_s":
                    interval = _as_float(value, "cdp.probe_interval_s")
                    config.cdp.probe_interval_s = _validate_positive_float(interval, "cdp.probe_interval_s")
        ...
        if "features" in loaded:
            for key, value in loaded["features"].items():
                if key == "token_align_frequency":
                    config.features.token_align_frequency = _as_int(value, "features.token_align_frequency")
...
def setup_profile_dir(profile_dir: pathlib.Path) -> None:
    ...
    try:
        ...
        _logger.error(
            ...
            profile_dir, user_name, group_name, uid, gid
        )
        sys.exit(EXIT_PROFILE_DIR_NOT_WRITABLE)
    except Exception as e:
    ...
    try:
        ...
        if actual_mode != 0o700:
            ...
            _logger.warning(
                ...
                profile_dir, mode_str, profile_dir
            )
    except Exception as e:
