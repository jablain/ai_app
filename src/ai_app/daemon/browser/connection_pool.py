...
class BrowserConnectionPool:
    ...
    async def _launch_browser(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Browser command not found: {self._cmd}. "
                f"Check cdp.cmd in config. Error: {e}"
            )
            return False
        except Exception as e:
    ...
    async def get_cdp_url(self) -> Optional[str]:
        if self._cdp_url:
            return self._cdp_url
        self._cdp_url = await self._discover_cdp_ws()
        return self._cdp_url

    async def _discover_cdp_ws(self) -> Optional[str]:
    ...
    async def get_page(self, cdp_url: str, page_url_hint: str) -> Optional[Page]:
        """
        ...
        async with self._lock:
            ...
            if self._cdp_url and cdp_url and (self._cdp_url != cdp_url):
                logger.debug("Requested CDP differs (using existing): %s != %s", cdp_url, self._cdp_url)
