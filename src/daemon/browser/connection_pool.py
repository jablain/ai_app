...
class BrowserConnectionPool:
    ...
    async def stop(self) -> None:
        ...
        async with self._lock:
                    ...
                    try:
                        ...
                        for context in contexts:
                            for page in context.pages:
                                try:
                                    await page.close()
                                    logger.debug(f"Closed page: {page.url}")
                                except Exception as e:
                                    logger.debug(f"Error closing page: {e}")
                        logger.info("All browser pages closed")
                    except Exception as e:
            ...
            # Run the blocking shutdown logic in a separate thread
            # to avoid freezing the main asyncio event loop.
            if self._browser_process:
                await asyncio.to_thread(self._shutdown_browser_sync)

            logger.info("Browser pool stopped")

    def _shutdown_browser_sync(self):
        """Synchronous helper for shutting down the browser."""
        ...
        try:
            ...
            # Check if we are running via flatpak
            if self._cmd.startswith("flatpak run"):
                try:
                    parts = shlex.split(self._cmd)
                    if len(parts) > 2:
                        app_id = parts[2]
                except Exception:
                    logger.warning("Failed to parse flatpak app_id, falling back to killpg")

            # Try to terminate gracefully
            try:
                if app_id:
                    logger.info(f"Sending graceful shutdown via flatpak kill {app_id}")
                    # Use flatpak kill to send SIGTERM *inside* the sandbox
                    subprocess.run(["flatpak", "kill", app_id], timeout=5, check=False)
                else:
                    logger.info(f"Sending SIGTERM to process group {pgid}")
                    os.killpg(pgid, signal.SIGTERM)

                # Wait for graceful shutdown
                self._browser_process.wait(timeout=10.0)  # Increased timeout
                ...
                logger.warning(f"Error during graceful termination: {e}")
                # Fallback to force kill if graceful fails
                try:
                    os.killpg(pgid, signal.SIGKILL)
                    self._browser_process.wait()
                    logger.info("Browser process group killed (fallback)")
                except Exception:
                    pass  # Already gone or permissions error
        except ProcessLookupError:
            logger.debug("Browser process already gone")
        except Exception as e:
    ...
    async def _launch_browser(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Browser command not found: {self._cmd}. Check cdp.cmd in config. Error: {e}"
            )
            return False
        except Exception as e:
