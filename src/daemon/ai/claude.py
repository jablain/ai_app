...
class ClaudeAI(BaseAI):
    """
    Transport-agnostic Claude adapter.

    - No Playwright/Web logic here.
    - Delegates all chat operations to the attached transport (e.g., ClaudeWebTransport).
    ...
    def attach_transport(self, transport: "ITransport") -> None:
        ...
        logger.info("ClaudeAI: transport attached -> %s", getattr(transport, "name", "unknown"))
        ...
        **kwargs,
        ...
        if not self._transport:
            meta = {
                "error": {
                    ...
                    "severity": "error",
                    "suggested_action": "Attach a transport (e.g., ClaudeWebTransport) at startup.",
                },
        ...
        if success and (markdown or snippet):
            try:
                ...
                logger.debug("ClaudeAI: session accounting failed: %s", e)
    ...
    async def list_messages(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("ClaudeAI.list_messages passthrough failed: %s", e)
        return []
    ...
    async def extract_message(self, baseline_count: int = 0) -> dict:
        ...
        try:
            ...
            logger.debug("ClaudeAI.extract_message passthrough failed: %s", e)
    ...
    async def start_new_session(self) -> bool:
        ...
        try:
            ...
            logger.debug("ClaudeAI.start_new_session passthrough failed: %s", e)
    ...
    def get_default_config(cls) -> Dict[str, Any]:
        """
        Keep the same defaults that worked previously for Claude.
        """
        return {
            "ai_target": "claude",
            "max_context_tokens": 200000,
            # No web selectors here anymore — that logic belongs to the transport.
            # Timings can be kept as metadata if a transport wants to read them.
            "response_wait_s": 60.0,
            "completion_check_interval_s": 0.3,
        }


# Register this adapter with the factory under the same key ('claude')
