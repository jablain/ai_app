...
class ChatGPTWebTransport(WebTransport):
    """
    Web transport tuned for ChatGPT's UI (selectors + overrides).
    ...
    async def _get_response_count(self, page: Page) -> int:
        """Count assistant message containers."""
        try:
    ...
    async def _send_message(self, page: Page, message: str) -> bool:
        ...
        try:
            ...
            # Try to click send button (becomes visible after text entered)
            try:
    ...
    async def _extract_response(self, page: Page, baseline_count: int) -> tuple[str, str]:
        """
        Extract last response from ChatGPT's UI.
        Steps:
          - wait up to RESPONSE_WAIT_S for response_count > baseline_count
          - select last RESPONSE_CONTENT
          - get inner_text()
        """
